...
export class MessageCommandEvent extends AnalyticsListener {
	public run(payload: MessageCommandSuccessPayload) {
		sharedRun(this, payload.command.name, CommandRunTypes.Message, payload.command.category);
	}
}
...
export class ChatInputCommandEvent extends AnalyticsListener {
	public run(payload: ChatInputCommandSuccessPayload) {
		sharedRun(this, payload.command.name, CommandRunTypes.ChatInput, payload.command.category);
	}
}
...
export class UserAnalyticsEvent extends AnalyticsListener {
	public run(payload: ContextMenuCommandSuccessPayload) {
		sharedRun(this, payload.command.name, CommandRunTypes.ContextMenu, payload.command.category);
	}
}
...
export class CommandAutocompleteEvent extends AnalyticsListener {
	public run(payload: AutocompleteInteractionPayload) {
		sharedRun(this, payload.command.name, CommandRunTypes.CommandAutocomplete, payload.command.category);
	}
}

function sharedRun(listener: AnalyticsListener, commandName: string, runType: CommandRunTypes, category: string | null): void {
