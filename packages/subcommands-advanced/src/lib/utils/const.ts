...
export const RegisterSubcommandsHooks = {
	subcommands: (piece: Subcommand, context: SlashCommandBuilder) => {
		...
		if (!subcommands) {
			container.logger.error(
				`[Subcommands] -> An attempt was made to obtain the subcommands for the parent command ${piece.name} but none were registered with the decorators.`
			);
	...
	groups: (piece: Subcommand, context: SlashCommandBuilder) => {
		...
		if (!subcommandsGroups) {
			container.logger.error(
				`[Subcommands group] -> An attempt was made to obtain the subcommand groups for the parent command ${piece.name} but none were registered with the decorators.`
			);
