...
export class Command<PreParseReturn extends Args = Args> extends SapphireCommand<PreParseReturn, SapphireCommand.Options & CommandOptions> {
	public constructor(context: SapphireCommand.LoaderContext, options: SapphireCommand.Options & CommandOptions) {
		...
		if (options.registerSubCommand && container.client.options.subcommandsAdvanced?.nameCommandsAutogenerated === true) {
			...
			if (options.registerSubCommand.messageSubCommand) {
				nameCommand = `${options.registerSubCommand.parentCommandName}/${options.registerSubCommand.messageSubCommand}`;
			}
		}
		...
		if (options.registerSubcommandInGroup && container.client.options.subcommandsAdvanced?.nameCommandsAutogenerated === true) {
			...
			if (options.registerSubcommandInGroup.messageSubCommand) {
				nameCommand = `${options.registerSubcommandInGroup.parentCommandName}/${options.registerSubcommandInGroup.groupName}/${options.registerSubcommandInGroup.messageSubCommand}`;
			}
		}
		...
		if (this.options.registerSubCommand) {
			const { parentCommandName, slashSubcommand, messageSubCommand } = this.options.registerSubCommand;
			analizeSubCommandParsed(this, parentCommandName, slashSubcommand ?? messageSubCommand);
		}

		if (this.options.registerSubcommandInGroup) {
			const { parentCommandName, groupName, slashSubcommand, messageSubCommand } = this.options.registerSubcommandInGroup;
			analizeSubcommandGroupParsed(this, parentCommandName, groupName, slashSubcommand ?? messageSubCommand);
		}
	}
}
